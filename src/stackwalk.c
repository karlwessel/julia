                    ...
                    size_t maxsize, int add_interp_frames) JL_NOTSAFEPOINT
{
    ...
    if (!jl_setjmp(buf, 0)) {
        ...
        while (1) {
           ...
           if (!jl_unw_step(cursor, &bt_entry->uintptr, &thesp, &thefp))
               break;
           if (sp)
               sp[n] = thesp;
           // FIXME cursor & bt_data remaining size handling doesn't work quite right here.
           if (add_interp_frames && jl_is_enter_interpreter_frame(bt_entry->uintptr)) {
               n += jl_capture_interp_frame(bt_entry, thesp, thefp, maxsize-n-1) + 1;
           } else {
               n++;
           }
        }
        ...
        while (n < jl_array_len(ip)) {
            ...
            if (!jl_bt_is_native(bt_entry)) {
                ...
                for (size_t j = 0; j < njlvals; j++)
                    jl_array_ptr_1d_push(bt2, bt_entry[1+j].jlvalue);
            }
